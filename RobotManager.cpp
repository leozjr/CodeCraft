...
void RobotManager::SingleManager(Robot * robots, WorkTable * tables, vector<vector<vector<pair<float, float> > > > & Road,int frameID)
{
	...
	if (frameID == 0) { //初始化
		...
		SetChooseTable(tables, Road); //买东西只能从中选择
		for (int i = 0; i < 4; i++) {
...
void RobotManager::SetChooseTable(WorkTable * tables, vector<vector<vector<pair<float, float> > > > & Road) ///有问题！！！！！！！！！！！！！！！！！！！
{
	this->m_ChooseTable.clear();
	for (int i = 0; i < this->m_TableNum; i++) {
...
void RobotManager::SetTargetTable(WorkTable * tables, Robot * robots, vector<vector<vector<pair<float, float> > > > & Road) { //从全局的角度设定应该卖到哪儿
...
void RobotManager::SetTargetTable_2(WorkTable * tables, Robot * robots, vector<vector<vector<pair<float, float> > > > & Road) { //从全局的角度设定应该买1，2，3
	int flagg[5] = { 0 };
	std::set<int> max_st;
	this->m_TargetTable.clear();
	for (int i = 0; i < this->m_TableNum; i++) {
